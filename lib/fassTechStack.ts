...
export const FASS_TECH_LAYERS: TechStackLayer[] = [
  {
    ...
    accent: "#00f0ff",
    items: [
      { name: "Next.js", detail: "React 기반 웹 애플리케이션을 만들기 위한 프론트엔드 프레임워크\n화면 라우팅, 페이지 구조, 빌드, 배포 구조를 담당", sprintId: "S04", assignee: "심지훈" },
      { name: "React 19", detail: "실제 화면을 구성하는 UI 개발 라이브러리\n버튼, 입력창, 메뉴, 팝업 같은 화면 요소를 컴포넌트 단위로 개발", sprintId: "S04", assignee: "심지훈" },
      { name: "Tailwind CSS", detail: "화면 디자인을 빠르게 적용하기 위한 스타일링 도구\n색상, 여백, 폰트, 버튼 모양 등을 일관되게 적용", sprintId: "S04", assignee: "심지훈" },
      { name: "RealGrid", detail: "업무시스템에서 대량 데이터를 조회·편집하기 위한 전문 그리드 컴포넌트", sprintId: "S04", assignee: "심지훈" },
      { name: "공통 UI 컴포넌트 라이브러리", detail: "회사 내부 시스템에서 반복적으로 쓰는 화면 요소를 표준화한 재사용 가능한 UI 묶음", sprintId: "S04", assignee: "심지훈" },
      ...
      { name: "Suspense", detail: "비동기 데이터 로딩 시 UI 렌더링 지연 및 스켈레톤 화면 처리", sprintId: "S09", assignee: "심지훈" },
      { name: "Next.js (SSR/RSC)", detail: "서버 사이드 렌더링을 통한 초기 로딩 속도 최적화 및 SEO 개선", sprintId: "S16", assignee: "심지훈" },
      { name: "Web Vitals", detail: "사용자 체감 웹 성능 지표(LCP, FID, CLS 등) 측정 및 개선", sprintId: "S16", assignee: "심지훈" },
    ],
  },
  {
    id: "backend",
    ...
    accent: "#3b82f6",
    items: [
      { name: "JWT", detail: "무상태(Stateless) 기반의 빠르고 안전한 사용자 인증 토큰 처리", sprintId: "S06" },
      { name: "Spring Security 6.x", detail: "엔터프라이즈 애플리케이션의 인증/인가를 통제하는 강력한 보안 필터", sprintId: "S06" },
      { name: "RBAC/ABAC", detail: "역할 및 속성(조건) 기반의 세밀한 사용자 메뉴/데이터 접근 통제", sprintId: "S07" },
      { name: "AOP", detail: "권한 검사 코드를 비즈니스 로직과 분리하여 모듈화(관점 지향 프로그래밍)", sprintId: "S07" },
      ...
      { name: "API Gateway", detail: "외부 API 요청을 단일 진입점에서 받아 라우팅, 인증, 트래픽 제어 수행", sprintId: "S12" },
      { name: "REST / gRPC", detail: "물류/유통 AI 모델과 백엔드 시스템 간의 빠르고 표준화된 통신 규격", sprintId: "S14", assignee: "송민준" },
      { name: "Spring Cloud", detail: "거대한 모놀리식 시스템을 독립적인 마이크로서비스로 분리하여 유연성 확보", sprintId: "S19" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "HashiCorp Vault", detail: "토큰, 비밀번호 등 민감 정보의 안전한 저장 및 동적 시크릿 관리", sprintId: "S03", assignee: "기충영" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Multi-tenancy", detail: "단일 시스템으로 여러 고객사(테넌트)의 데이터를 DB/스키마 레벨에서 안전하게 격리", sprintId: "S11" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Eureka (Discovery)", detail: "마이크로서비스 등록 및 탐색을 통한 동적 라우팅 지원", sprintId: "S03", assignee: "기충영" },
      { name: "ClipReport", detail: "엔터프라이즈급 리포팅 솔루션을 활용한 전문 업무 서식(명세서, 대장 등) 설계 및 리포트 출력", sprintId: "S23", assignee: "김희찬" },
      { name: "Browser Print / PDF", detail: "브라우저 표준 인쇄 기능 및 화면 HTML 기반의 PDF 변환/인쇄 기능 제공", sprintId: "S23", assignee: "김희찬" },
      { name: "API Management", detail: "전사 API의 생성, 게시, 모니터링, 과금 등 전체 생명주기 통합 관리", sprintId: "S13" },
